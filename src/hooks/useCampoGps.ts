...
export function useCampoGps(comercioId:string|null, ordenId:string|null, parteId:string|null, canView:boolean, canOperate:boolean) {
  ...
  const change=useMutation({mutationFn:async(action:"pausar"|"reanudar"|"finalizar")=>{const s=sessionRef.current;if(!canOperate||!s)throw new Error("Sesión no disponible");if(action!=="pausar")await flush(s);const {data,error:rpcError}=await supabase.rpc("campo_gps_cambiar_estado",{p_sesion_id:s.id,p_accion:action});if(rpcError)throw rpcError;return data as CampoGpsSesion;},onSuccess:(s,action)=>{qc.setQueryData(sessionKey(comercioId,parteId),s);if(action==="pausar"||action==="finalizar")stopWatch();else startWatch(s);}});
