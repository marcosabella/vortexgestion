...
export function useCampoGps(comercioId:string|null, ordenId:string|null, parteId:string|null, canView:boolean, canOperate:boolean) {
  ...
  const start=useMutation({mutationFn:async(parteMaquinariaId?:string|null)=>{if(!canOperate||!parteId)throw new Error("Parte no disponible");const {data,error:rpcError}=await supabase.rpc("campo_gps_iniciar_sesion",{p_parte_id:parteId,p_parte_maquinaria_id:parteMaquinariaId??undefined});if(rpcError)throw rpcError;return data as CampoGpsSesion;},onSuccess:s=>{qc.setQueryData(sessionKey(comercioId,parteId),s);setError(null);startWatch(s);}});
