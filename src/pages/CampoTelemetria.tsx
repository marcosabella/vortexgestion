...
import { useCampoTelemetria, useCampoTelemetriaDetalle, useCampoTelemetriaImportar, useCampoTelemetriaRuta } from "@/hooks/useCampoTelemetria";
...
function FitRoute({ points }: { points: [number, number][] }) { const map=useMap(); useEffect(()=>{if(points.length===1) map.setView(points[0],16); else if(points.length>1) map.fitBounds(points,{padding:[20,20]});},[map,points]); return null; }
function RouteMap({ points }: { points: [number, number][] }) { return <div className="h-80 overflow-hidden rounded-md border"><MapContainer center={points.at(-1)!} zoom={16} className="h-full w-full" scrollWheelZoom><TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/><Polyline positions={points} pathOptions={{color:"#2563eb",weight:4}}/><CircleMarker center={points[0]} radius={6} pathOptions={{color:"#16a34a"}}/><CircleMarker center={points.at(-1)!} radius={6} pathOptions={{color:"#dc2626"}}/><FitRoute points={points}/></MapContainer></div>; }
export function CampoTelemetriaDetalle({ comercioId, id, allowed }: { comercioId: string; id: string; allowed: boolean }) {
  const { estado, resumen } = useCampoTelemetriaDetalle(comercioId, id, allowed);
  const ruta = useCampoTelemetriaRuta(comercioId, id, allowed);
  ...
  const routePoints=useMemo(()=>ruta.data?.map(point=>[point.latitud,point.longitud] as [number,number])??[],[ruta.data]);
...
export default function CampoTelemetria() {
  ...
  const initialTab = searchParams.get("tab") === "importaciones" ? "importaciones" : "importar";
