...
export function ParteInsumos({ comercioId, ordenId, parteId, access, isAdmin, orden, parte }: { comercioId: string; ordenId: string; parteId: string; access: boolean; isAdmin: boolean; orden: CampoOrdenDetail; parte: CampoParte }) {
  ...
  return <Card>
    ...
    <CardContent>
      {query.isLoading || catalog.isLoading ? <p>Cargando insumos...</p> : query.error || catalog.error ? <p className="text-destructive">No se pudieron cargar los insumos.</p> : !query.data?.length ? <p className="text-muted-foreground">Todavía no hay insumos en el parte.</p> : <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{query.data.map((item) => <Card key={item.id} className={!item.activo ? "opacity-70" : ""}><CardContent className="space-y-2 pt-4">
