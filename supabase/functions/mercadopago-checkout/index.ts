...
Deno.serve(async (req) => {
  ...
  try {
    const user = await authenticatedUser(req, db),
      body = await req.json(),
      comercioId = String(body.comercioId || ""),
    ...
    const { data: config } = await db.from("mercadopago_configuraciones")
      .select("*").eq("comercio_id", comercioId).eq("connected", true).eq(
        "checkout_habilitado",
        true,
      ).single();
    if (!config) {
      throw new Error("Mercado Pago no esta habilitado para este comercio");
    }
    const cred = await credential(db, comercioId),
      ref = externalReference(comercioId, "pedido", pedido.id),
    ...
    const allowedOrigins = (Deno.env.get("MP_ALLOWED_STORE_ORIGINS") || "")
      .split(",").map((v) => v.trim()).filter(Boolean);
    const isLocalDevelopmentOrigin = origin.startsWith("http://localhost") ||
      origin.startsWith("http://127.0.0.1");
    if (
      (!origin.startsWith("https://") &&
        !isLocalDevelopmentOrigin) ||
      (allowedOrigins.length && !isLocalDevelopmentOrigin &&
