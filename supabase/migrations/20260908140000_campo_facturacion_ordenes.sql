-- Vortex Campo: facturacion administrativa de ordenes finalizadas.
-- Reutiliza registrar_venta_transaccional: no crea caja, CAE/ARCA ni stock.
BEGIN;

-- Esta clave candidata es necesaria para que la FK de venta tambien valide
-- comercio. id es PK global, por lo que no introduce colisiones de datos.
ALTER TABLE public.ventas
  ADD CONSTRAINT ventas_comercio_id_id_key UNIQUE (comercio_id, id);

DO $do$
BEGIN
  IF to_regprocedure('extensions.digest(bytea,text)') IS NULL THEN
    RAISE EXCEPTION 'campo_facturacion_digest_sha256_no_disponible'
      USING ERRCODE = '42883',
            HINT = 'Habilitar pgcrypto en el schema extensions antes de aplicar la migracion.';
  END IF;
END;
$do$;

CREATE TABLE public.campo_orden_comprobantes (
  ...
  huella_pedido character(64) NOT NULL,
  estado text NOT NULL DEFAULT 'vigente',
  created_by uuid NOT NULL DEFAULT auth.uid(),
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT campo_orden_comprobantes_moneda_valida CHECK (moneda IN ('ARS', 'USD')),
  CONSTRAINT campo_orden_comprobantes_huella_valida CHECK (huella_pedido ~ '^[0-9a-f]{64}$'),
  CONSTRAINT campo_orden_comprobantes_estado_valido CHECK (estado = 'vigente'),
  CONSTRAINT campo_orden_comprobantes_orden_fkey
    FOREIGN KEY (comercio_id, orden_id)
    REFERENCES public.campo_ordenes_trabajo(comercio_id, id) ON DELETE RESTRICT,
  CONSTRAINT campo_orden_comprobantes_venta_fkey
    FOREIGN KEY (comercio_id, venta_id)
    REFERENCES public.ventas(comercio_id, id) ON DELETE RESTRICT,
  CONSTRAINT campo_orden_comprobantes_orden_moneda_key UNIQUE (comercio_id, orden_id, moneda),
  CONSTRAINT campo_orden_comprobantes_venta_key UNIQUE (venta_id),
  CONSTRAINT campo_orden_comprobantes_idempotency_key
    UNIQUE (comercio_id, idempotency_key)
);

ALTER TABLE public.campo_orden_comprobantes ENABLE ROW LEVEL SECURITY;

CREATE POLICY campo_orden_comprobantes_select_admin
ON public.campo_orden_comprobantes
FOR SELECT TO authenticated
USING (
  auth.uid() IS NOT NULL
  AND public.user_is_comercio_admin(comercio_id)
);

REVOKE ALL ON TABLE public.campo_orden_comprobantes FROM PUBLIC, anon, authenticated;
GRANT SELECT ON TABLE public.campo_orden_comprobantes TO authenticated;

CREATE FUNCTION public.campo_facturar_orden(
  p_orden_id uuid,
  p_moneda text,
...
RETURNS TABLE (
  venta public.ventas,
  comprobante public.campo_orden_comprobantes,
  moneda character(3),
  subtotal numeric,
  total_iva numeric,
  total numeric
...
BEGIN
  ...
  -- Todas las operaciones sobre una orden adquieren primero este bloqueo.
  ...
  -- Serializa tambien el primer uso de una clave entre ordenes diferentes.
  PERFORM pg_advisory_xact_lock(hashtextextended(
    'campo:facturacion:idempotency:' || v_orden.comercio_id::text || ':' || p_idempotency_key::text,
    0
  ));

  -- Despues de la orden, el vinculo se bloquea siempre en la misma secuencia.
  ...
  IF v_comprobante.id IS NOT NULL THEN
    ...
    WHERE v.id = v_comprobante.venta_id
      AND v.comercio_id = v_comprobante.comercio_id;
  ...
  WITH ejecuciones AS (
    SELECT
      ...
      l.porcentaje_iva_snapshot,
      CASE
        WHEN l.unidad = 'fijo' THEN 1::numeric
        ELSE round(sum(pl.cantidad_ejecutada), 4)
      END AS cantidad
    ...
    SELECT e.*,
      round(e.cantidad * e.precio_unitario_snapshot, 2) AS total_linea
    FROM ejecuciones AS e
  )
  SELECT
    jsonb_agg(
      jsonb_strip_nulls(jsonb_build_object(
        'descripcion_manual', nombre,
        'codigo_manual', codigo_interno,
        'cantidad', cantidad,
        'precio_unitario', precio_unitario_snapshot,
        'porcentaje_iva', porcentaje_iva_snapshot,
        'afecta_stock', false
      ))
      ORDER BY posicion, id
    ),
    round(sum(total_linea), 2)
  ...
  IF p_modalidad = 'transferencia' THEN
    v_pagos := jsonb_build_array(jsonb_build_object(
      ...
      'monto', v_total_estimado
    ));
  ELSE
    v_pagos := '[]'::jsonb;
  END IF;

  -- Esta es la unica alta de venta: mantiene numeracion, pagos y cuenta
  -- corriente bajo el contrato transaccional ya vigente.
  SELECT * INTO v_venta
...
CREATE OR REPLACE FUNCTION public.campo_cambiar_estado_orden(
  p_orden_id uuid,
  p_nuevo_estado text,
  p_motivo text DEFAULT NULL
)
RETURNS public.campo_ordenes_trabajo
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public
AS $function$
...
BEGIN
  ...
  IF p_nuevo_estado = 'cancelada' AND EXISTS (
    SELECT 1 FROM public.campo_orden_comprobantes AS c
    WHERE c.comercio_id = v_orden.comercio_id
      AND c.orden_id = v_orden.id
      AND c.estado = 'vigente'
  ) THEN
    RAISE EXCEPTION 'campo_orden_facturada_no_cancelable' USING ERRCODE = '23514';
  END IF;
...
REVOKE ALL ON FUNCTION public.campo_cambiar_estado_orden(uuid, text, text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.campo_cambiar_estado_orden(uuid, text, text)
  TO authenticated;

COMMENT ON TABLE public.campo_orden_comprobantes IS
  'Vinculo append-only de una venta administrativa vigente por orden Campo y moneda. ARS y USD nunca se suman ni convierten.';
COMMENT ON FUNCTION public.campo_facturar_orden(uuid, text, public.tipo_comprobante, integer, text, uuid) IS
  'Factura labores Campo confirmadas y facturables por moneda, reutilizando registrar_venta_transaccional. Solo administradores; transferencia genera un unico pago transferencia y cta_cte un unico debito. Sin caja, stock, Mercado Pago ni CAE/ARCA.';
