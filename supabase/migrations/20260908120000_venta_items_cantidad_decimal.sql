-- Permite cantidades decimales exclusivamente en lineas que no afectan stock.
-- Se usa numeric sin typmod para que una escala mayor a cuatro se rechace por
-- constraint, en vez de ser redondeada silenciosamente por numeric(14,4).
BEGIN;

-- Es el unico trigger de venta_items cuya lista UPDATE OF depende de cantidad.
-- Los demas triggers vigentes son INSERT, UPDATE sin lista de columnas o no
-- refieren cantidad en su definicion, por lo que no bloquean el ALTER TYPE.
...
ALTER TABLE public.venta_items
  ...
  ALTER COLUMN cantidad SET DEFAULT 1::numeric;

ALTER TABLE public.venta_items
  DROP CONSTRAINT IF EXISTS venta_items_cantidad_valida,
  DROP CONSTRAINT IF EXISTS venta_items_stock_coherente;

-- Backfill unico y acotado: las lineas manuales historicas anteriores a
-- afecta_stock heredaron true pese a no tener producto. El trigger vigente
-- aplica_venta_item_stock, en la transicion true->false, solo repone si OLD
-- producto_id no es NULL; estas filas no consultan ni modifican productos.
...
ALTER TABLE public.venta_items
  ADD CONSTRAINT venta_items_cantidad_valida CHECK (
    cantidad > 0
    AND cantidad::text NOT IN ('NaN', 'Infinity', '-Infinity')
    AND scale(cantidad) <= 4
    AND cantidad <= 9999999999.9999::numeric
  ),
  ADD CONSTRAINT venta_items_stock_coherente CHECK (
    (afecta_stock AND producto_id IS NOT NULL AND cantidad = trunc(cantidad))
    OR (NOT afecta_stock AND producto_id IS NULL)
  );
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    UPDATE public.productos SET stock = stock - NEW.cantidad::integer
    WHERE id = NEW.producto_id AND stock >= NEW.cantidad::integer;
  ...
  ELSIF TG_OP = 'DELETE' THEN
    IF OLD.afecta_stock AND OLD.producto_id IS NOT NULL THEN
      UPDATE public.productos SET stock = stock + OLD.cantidad::integer WHERE id = OLD.producto_id;
    END IF;
    RETURN OLD;
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.afecta_stock AND NOT NEW.afecta_stock THEN
      IF OLD.producto_id IS NOT NULL THEN UPDATE public.productos SET stock = stock + OLD.cantidad::integer WHERE id = OLD.producto_id; END IF;
      RETURN NEW;
    END IF;
    ...
    IF OLD.producto_id = NEW.producto_id THEN
      IF NEW.cantidad > OLD.cantidad THEN
        UPDATE public.productos SET stock = stock - (NEW.cantidad - OLD.cantidad)::integer
        WHERE id = NEW.producto_id AND stock >= (NEW.cantidad - OLD.cantidad)::integer;
        GET DIAGNOSTICS affected_rows = ROW_COUNT;
        IF affected_rows = 0 THEN RAISE EXCEPTION 'Stock insuficiente para el producto seleccionado'; END IF;
      ELSIF NEW.cantidad < OLD.cantidad THEN
        UPDATE public.productos SET stock = stock + (OLD.cantidad - NEW.cantidad)::integer WHERE id = NEW.producto_id;
      END IF;
    ELSE
      UPDATE public.productos SET stock = stock + OLD.cantidad::integer WHERE id = OLD.producto_id;
      UPDATE public.productos SET stock = stock - NEW.cantidad::integer
...
BEGIN
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) LOOP
    ...
    v_precio := (v_item->>'precio_unitario')::numeric; v_iva_pct := (v_item->>'porcentaje_iva')::numeric;
